...
function patch(dir) {
  ...
  fs.readdirSync(dir, { withFileTypes: true }).forEach(e => {
    ...
    if (e.isDirectory()) patch(p);
      ...
      if (c.includes('__vite_rsc_assets_manifest.js')) {
        c = c.replaceAll('__vite_rsc_assets_manifest.js', './__vite_rsc_assets_manifest.js');
...
patch('./dist');
console.log('RSC manifest patch applied successfully.');
"
